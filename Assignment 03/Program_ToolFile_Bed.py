...
def write_file():  # function for write File
    ...
    while True: # This function is used to check the data, if correct it will continue.
        try:
            data_input = int(input('Enter a number :')) # Define data information Get data from the keyboard
            data_list.append(str(data_input)) # Keep the received data in data_list
        except:
            print('Please Enter a number') # This function is for comparison if input from the keyboard other than set. The message will be displayed 
            break
